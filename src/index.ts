...
export class TypeScriptPlugin {
	...
	constructor(serverless: ServerlessInstance, options: ServerlessOptions) {
		...
		// @ts-ignore
		const pluginOptions = this.serverless.service.custom && this.serverless.service.custom[ 'serverless-plugin-typescript' ];
		const customOptions = pluginOptions && pluginOptions.projectRoot ? pluginOptions.projectRoot : '';
		this.projectRoot = path.join(this.serverless.config.servicePath, customOptions);
	...
	async copyExtras() {
		...
		if (!fs.existsSync(path.resolve(path.join(buildFolder, 'node_modules')))) {
			fs.symlinkSync(path.resolve(path.join(this.projectRoot, 'node_modules')), path.resolve(path.join(buildFolder, 'node_modules')));
		}

		// include package.json into build so Serverless can exclude devDeps during packaging
		if (!fs.existsSync(path.resolve(path.join(buildFolder, 'package.json')))) {
			fs.symlinkSync(path.resolve(path.join(this.projectRoot, 'package.json')), path.resolve(path.join(buildFolder, 'package.json')));
		}
